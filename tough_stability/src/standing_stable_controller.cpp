#include <tough_stability/standing_stable_controller.h>

stableController::stableController(ros::NodeHandle nh, bool vis_flag) : nh_(nh), vis_flag_(vis_flag)
{
...
void stableController::initSolvers(void)
{
  ...
  //    {
  //        std::cout<< "segment: " << chain_.getSegment(i).getName() << " joint " <<
  //        chain_.getSegment(i).getJoint().getName() <<std::endl;
  //    }
  std::cout << std::endl;
  chain_.addSegment(
      KDL::Segment("com_pelvis", KDL::Joint("rightHipYaw", KDL::Joint::RotZ),
                   KDL::Frame(KDL::Vector(com_wrt_root().getX(), com_wrt_root().getY(), com_wrt_root().getZ()))));
...
void stableController::updateCOM(void)
{
  ...
  {
    joint_positions_.insert(
        make_pair(joint_state_->getJointStates().name[i], joint_state_->getJointStates().position[i]));
    // std::cout<< "name: " << joint_state_->getJointStates().name[i] << " pos: " <<
    // joint_state_->getJointStates().position[i]<<std::endl;
  }
  ...
  ROS_INFO("projected com: x %f, y: %f, z: %f", stability_->getCOM().getX(), stability_->getCOM().getY(),
           stability_->getCOM().getZ());
...
void stableController::updateJntVelocities(void)
{
  for (int i = 0; i < chain_.getNrOfSegments(); i++)
  {
    ...
    // std::cout << "pos " << q_(i) << " joint name " << chain_.getSegment(i).getJoint().getName() << " segment name "
    // << chain_.getSegment(i).getName() << " act pos "<<
    // joint_positions_.find(chain_.getSegment(i).getJoint().getName())->second << std::endl;
    file_ << q_(i) << ",";
  }
  std::cout << std::endl;
...
void stableController::computeJointTorques(void)
{
  ...
  // cap on tiorquw
  jntTorques_(5) =
      (fabs(jntTorques_(5)) > 1000.0 ? copysign(1000.0, ((jntTorques_(5) > 0) ? 1.0 : -1.0)) : jntTorques_(5));
  jntTorques_(4) =
      (fabs(jntTorques_(4)) > 1000.0 ? copysign(1000.0, ((jntTorques_(4) > 0) ? 1.0 : -1.0)) : jntTorques_(4));
  jntTorques_(3) =
      (fabs(jntTorques_(3)) > 1000.0 ? copysign(1000.0, ((jntTorques_(3) > 0) ? 1.0 : -1.0)) : jntTorques_(3));
  jntTorques_(2) =
      (fabs(jntTorques_(2)) > 1000.0 ? copysign(1000.0, ((jntTorques_(2) > 0) ? 1.0 : -1.0)) : jntTorques_(2));
  jntTorques_(1) =
      (fabs(jntTorques_(1)) > 1000.0 ? copysign(1000.0, ((jntTorques_(1) > 0) ? 1.0 : -1.0)) : jntTorques_(1));
  jntTorques_(0) =
      (fabs(jntTorques_(0)) > 1000.0 ? copysign(1000.0, ((jntTorques_(0) > 0) ? 1.0 : -1.0)) : jntTorques_(0));
...
std::vector<double> stableController::getThetas()
{
  ...
  std::cout << "original com: " << com_wrt_root().getX() << " " << com_wrt_root().getY() << " " << com_wrt_root().getZ()
            << std::endl;
  std::cout << "rtransformed com: " << rfoot_point_transformed.point.x << " " << rfoot_point_transformed.point.y << " "
            << rfoot_point_transformed.point.z << std::endl;
  std::cout << "ltransformed com: " << lfoot_point_transformed.point.x << " " << lfoot_point_transformed.point.y << " "
            << lfoot_point_transformed.point.z << std::endl;

  // transform the right foot to pelvis frame
  double rtheta =
      acos(rfoot_point_transformed.point.y / sqrt(rfoot_point_transformed.point.x * rfoot_point_transformed.point.x +
                                                  rfoot_point_transformed.point.y * rfoot_point_transformed.point.y));
  double ltheta =
      acos(lfoot_point_transformed.point.y / sqrt(lfoot_point_transformed.point.x * lfoot_point_transformed.point.x +
                                                  lfoot_point_transformed.point.y * lfoot_point_transformed.point.y));
