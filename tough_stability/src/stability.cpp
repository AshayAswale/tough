#include <tough_stability/stability.h>

// constructor

stability::stability(float foot_polygon_scale, ros::NodeHandle nh, std::string foot_mesh_link, std::string root_link,
                     std::string rfoot, std::string lfoot)
  : support_polygon_scale_(foot_polygon_scale)
  , nh_(nh)
  , foot_mesh_link_(foot_mesh_link)
...
bool stability::constructSupportPolygon(void)
{
  ...
  tf::Pose geom_origin = tf::Pose(tf::Transform(
      tf::Quaternion(geom_pose.rotation.x, geom_pose.rotation.y, geom_pose.rotation.z, geom_pose.rotation.w),
  ...
  else
  {
    ...
    for (unsigned int i = 0; i < vertex_count; ++i)
    {
      ...
      tf::Point p(shape_mesh->vertices[i3], shape_mesh->vertices[i3 + 1],
                  shape_mesh->vertices[i3 + 2]);  // proj down (z=0)
...
std::vector<tf::Point> stability::convexHull(const std::vector<tf::Point>& points) const
{
  ...
  for (unsigned i = 0; i < polygons[0].vertices.size(); ++i)
  {
    ...
    tf::Point p(chull_points.points[idx].x, chull_points.points[idx].y, chull_points.points[idx].z);
    hull.push_back(p);
  }
