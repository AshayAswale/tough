...
com::com(ros::NodeHandle& nh, std::string root_link_name, std::string rfoot_link_name, std::string lfoot_link_name,
         bool enableVis)
...
void com::init(void)
{
  rd = RobotDescription::getRobotDescription(nh_);
  std::string robot_descr_param_name = prefix + rd->getRobotName() + "/robot_description";
  nh_.getParam(robot_descr_param_name, robot_desc_string_);
...
void com::getCOMForLink(const KDL::SegmentMap::const_iterator& current_seg,
                        const std::map<std::string, double>& joint_positions, const KDL::Frame& tf,
                        KDL::Frame& tf_right_foot, KDL::Frame& tf_left_foot, double& m, KDL::Vector& com)
{
  double jnt_p = 0.0;
  ...
  if (current_seg->second.segment.getJoint().getType() != KDL::Joint::None)
  {
    std::map<std::string, double>::const_iterator jnt =
        joint_positions.find(current_seg->second.segment.getJoint().getName());
  ...
  m += current_m;
  // ROS_INFO("At link %s. local: %f / [%f %f %f]; global: %f / [%f %f %f]",current_seg->first.c_str(), current_m,
  ...
  if (isEnableVis())
  {
    if (current_m > 0.0)
    {
      ...
      // <<com_vis_markers_.markers.size() << std::endl;
...
void com::createCOMMarker(const std::string& ns, const std::string& frame_id, double radius, const KDL::Vector& cog,
                          visualization_msgs::Marker& marker)
{
  ...
  if (getKDLTree())
  {
    ...
    for (it = segment_map.begin(); it != segment_map.end(); it++)
    {
      std::cout << i << " " << it->first << ' ' << it->second.segment.getInertia().getCOG()[0] << ' '
                << it->second.segment.getInertia().getCOG()[1] << ' ' << it->second.segment.getInertia().getCOG()[2]
...
void humanoidCommon::getCOMForLink(std::string link_name, KDL::Tree tree,
                                   KDL::ChainFkSolverPos_recursive* tree_fk_solver, float* com_x, float* com_y,
                                   float* com_z, float* mass)
{
